...
function spremiPorukeUStorage() {
    const mapa = {};
    sviKorisnici.forEach(k => mapa[k.id] = k.poruka || "");
    localStorage.setItem('porukePolaznika', JSON.stringify(mapa));
}
...
//----------------------------------------------------------------------
    const poljeAudio = document.createElement('td');

    const audioPlayback = document.createElement('audio');
    audioPlayback.controls = true;

    const startBtn = document.createElement('button');
    startBtn.textContent = '🎙️ Snimi';
    ...
    poljeAudio.appendChild(audioPlayback);
    poljeAudio.appendChild(document.createElement('br'));
    poljeAudio.appendChild(startBtn);
    poljeAudio.appendChild(stopBtn);

    red.appendChild(poljeAudio);
